...
def index():
    ...
    stock = db.execute("SELECT shares,price FROM transactions WHERE user_id = ?", user_id)
    stock_money = 0
    for i in stock:
        stock_money += i["price"] * i["shares"]

    total = cash[0]["cash"]  + stock_money
...
def fund():
    ...
    user_id = session["user_id"]
    user_cash = db.execute("SELECT cash FROM users WHERE id=?", user_id)
    updated = int(request.form.get("money"))
    db.execute("UPDATE users SET cash=? WHERE id=?",int(user_cash[0]["cash"]+updated),user_id)
    flash("Money Added")
    return redirect("/")
